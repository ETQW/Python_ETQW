...
while True:
    choose = input(
        "Для аналитики введите 'A', для того, чтобы продолжить нажмите 'Enter', а для выхода наберите 'Q'").upper()
    if choose == 'Q':
        break
    n += 1
    if choose == 'A':
    ...
    for f in params.keys():
        param_input = input(f'Заведите параметр товара: "{f}"\n')
        params[f] = int(param_input) if (f == 'цена' or f == 'количество') else param_input
        analytics[f].append(params[f])
    goods.append((n, params))
